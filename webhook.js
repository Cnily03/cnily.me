...
http.createServer((req, res) => {
    ...
    req.on('end', () => {
        try {
            if (verify(req, res, chunks)) { // 验证
                handler(req, res, chunks) // 处理
                console.log(`ACCEPT ${req.method} ${req.url}`)
            } else throw new Error("Unexpected Request")
        } catch (e) {
            if (e.message === "Unexpected Request") {
                res.statusCode = 502
                res.end("Verification Failed")
            } else {
                console.log(e)
                res.statusCode = 502
                res.end("Rejection due to unkown reason")
            }
...
function verify(req, res, chunks) {
    // verify header
    const request = locationInfo(req)
    var t = req.method == "POST" &&
        ...
        (!CONFIG.verify.content_type.length || CONFIG.verify["content-type"].includes(req.headers.content_type)) &&
        (!CONFIG.verify.x_github_event.length || CONFIG.verify["x-github-event"].includes(req.headers.x_github_event))
    if (!t) return false
    // verify body
    var data = req.headers.content_type == "application/json" ?
...
function handler(req, res, chunks) {
    ...
    if (Date.now() >= latest_call_time + CONFIG.ttl) {
        ...
        res.statusCode = 202
        res.end("Successfully call commands on the server")
    } else {
        res.statusCode = 200
        res.end("Request in TTL")
        console.log('[WARN] Passing one request due to TTL')
    }
}

console.log("Sever is running at port " + CONFIG.server_port + "...")